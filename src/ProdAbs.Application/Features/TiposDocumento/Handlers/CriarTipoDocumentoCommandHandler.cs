...
public class
    CriarTipoDocumentoCommandHandler : IRequestHandler<CriarTipoDocumentoCommand, Result<TipoDocumentoDetalhesDTO>>
{
    ...
    public async Task<Result<TipoDocumentoDetalhesDTO>> Handle(CriarTipoDocumentoCommand request,
        CancellationToken cancellationToken)
    {
        var tipoDocumento = new TipoDocumento(
            Guid.NewGuid(),
            request.Nome,
            request.Campos.Select(c =>
                    new CampoMetadata(c, new RegraValidacao(TipoDeDados.String, false, string.Empty), string.Empty))
                .ToList()
        );
